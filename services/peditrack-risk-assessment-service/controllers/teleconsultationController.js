...
exports.acceptRequest = async (req, res) => {
  try {
    ...
    if (!updated) {
      const existingCheck = await TeleconsultationRequest.findById(id);
      if (!existingCheck) return res.status(404).json({ message: 'Request not found' });
      if (existingCheck.status === 'accepted' && existingCheck.doctorId === doctorId) {
        return res.json(existingCheck);
      }
      return res.status(409).json({ message: `Request already ${existingCheck.status}` });
    }
    res.json(updated);
  } catch (err) {
